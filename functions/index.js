    ...
    try {
      cookieParser()(req, res, () => {
        ...
        functions.logger.log(
          ...
          functions.config().spotify.redirect_uri,
          req.query.code
        );
        ...
        Spotify.authorizationCodeGrant(req.query.code, (error, data) => {
          if (error) {
            functions.logger.log("Spotify auth issue", error.message);
            throw error;
          }
          functions.logger.log(
    ...
    cors(req, res, () => {
      validateFirebaseIdToken(req, res, async () => {
        ...
        const userTokenData = await admin
          .firestore()
          .doc(`/spotifyAccessToken/${uid}`)
          .get();
        functions.logger.log("Existing user token data fetched", userTokenData);
...
async function createFirebaseAccount(
  ...
  const sessionCreateTask = admin
    .firestore()
    .doc(`/sessions/${uid}`)
    .set({ displayName, admin: uid, members: [uid] }, { merge: true });
